...
void DepthExporter::exportCloud(string filename, vector < ofPoint > pts){
	...
	if (ply.is_open()) {
		...
		for (int i = 0; i < pts.size(); i++) {
			exportPlyVertex(vertices, pts[i].x, pts[i].y, pts[i].z, NULL);
			total ++;
		}
...
void DepthExporter::exportCloud(string filename, vector < ofPoint > pts, vector < ofPoint > colors){
	...
	if (ply.is_open()) {
		...
		for (int i = 0; i < pts.size(); i++) {
			...
			colorPixels[0] = colors[i].x;
			colorPixels[1] = colors[i].y;
			colorPixels[2] = colors[i].z;
			
			exportPlyVertex(vertices, pts[i].x, pts[i].y, pts[i].z, colorPixels);
			total ++;
		}
