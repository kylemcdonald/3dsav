...
void DepthVisualizerInput::buildPointCloud() {	
	...
	if(useKinect) {
		...
		unsigned char* colorPixelsAlligned = kinect.getCalibratedRGBPixels();
		...
		for(int y = 0; y < camHeight; y++) {
			for(int x = 0; x < camWidth; x++) {
				...
				if(depthPixels[i] != 0) {
					...
					unsigned char red = colorPixelsAlligned[(y*640+x)*3 + 0];
					unsigned char green = colorPixelsAlligned[(y*640+x)*3 + 1];
					unsigned char blue = colorPixelsAlligned[(y*640+x)*3 + 2];
					
					pointCloudColors.push_back(ofPoint(red, green, blue));
				}
			}
		}
	} else {
